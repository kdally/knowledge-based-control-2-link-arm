...
for i = 1:length(rot_vel)
    if any([1,2,3,4,8,10] == i) 
        Q1 = [((rot_vel(i)-lx(1))/12.9+px(1)).*[1., 1.], (RMSE_DYN1.x(i)-ly(1))/0.18+py(1)+0.05, (RMSE_DYN1.x(i)-ly(1))/0.18+py(1)];
    else
        Q1 = [((rot_vel(i)-lx(1))/12.9+px(1)).*[1., 1.], (RMSE_DYN1.x(i)-ly(1))/0.18+py(1)-0.05, (RMSE_DYN1.x(i)-ly(1))/0.18+py(1)];
    end
...
% Add initial offset values as annotations
for i = 1:length(rot_vel)
    if any([1,2,3,4,6,8,10] == i) 
        Q1 = [((rot_vel(i)-lx(1))/12.9+px(1)).*[1., 1.], (RMSE_yours.th(i)-ly(1))/0.07+py(1)+0.07, (RMSE_yours.th(i)-ly(1))/0.07+py(1)];
    else
        Q1 = [((rot_vel(i)-lx(1))/12.9+px(1)).*[1., 1.], (RMSE_yours.th(i)-ly(1))/0.07+py(1)-0.07, (RMSE_yours.th(i)-ly(1))/0.07+py(1)];
    end
    annotation('textarrow', Q1(1:2), Q1(3:4), 'String',strcat('',num2str(initial_offset_vector(1,i)),'°,  ',num2str(initial_offset_vector(2,i)),'°'),'TextBackgroundColor','white','TextEdgeColor',[0.4 0.4 0.4]);
